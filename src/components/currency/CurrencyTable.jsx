...
import CurrencyLayout from './CurrencyTable.style'
...
const CurrencyTable = () => {
  ...
  const fetch = () =>
  axios
    .get(`https://api.privatbank.ua/p24api/exchange_rates?json&date=31.07.2022`)
      .then(({ data }) => {
        data && setData(data.exchangeRate.filter(({currency}) => baseCurrency.includes(currency)))
  ...
  <CurrencyLayout>
    <TableContainer>
      ...
      <tbody>
        {dataCurrency && dataCurrency.map(({ currency, saleRateNB, purchaseRateNB }, index) => (
